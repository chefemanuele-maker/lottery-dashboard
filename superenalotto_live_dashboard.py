...
try:
    ...
    print("This app needs xlrd installed to read your .xls archives. Try: pip3 install xlrd")
    raise
...
def load_local_history() -> pd.DataFrame:
    ...
    if LOCAL_HISTORY.exists():
        try:
            frames.append(standardize_history(pd.read_csv(LOCAL_HISTORY)))
        except Exception:
            pass
    ...
    if not frames:
        raise FileNotFoundError(
            "No SuperEnalotto history found. Keep the .xls archives near the dashboard or in Downloads/Desktop."
        )
